...
public class AtCoderScraper implements WebScraper {
    ...
    private static final String CONTESTS_PAGE = "/contests";
    
    @Override
    public List<ScrapedContestItem> scrape() {
        ScraperProperties.ScraperSourceConfig config = properties.getSources().get("atcoder");
        if (config == null) {
            throw new ScrapingException("AtCoder scraper source configuration not found");
        }
        
        // robots.txt 확인
        if (!robotsTxtChecker.isAllowed(config.getBaseUrl(), CONTESTS_PAGE)) {
            throw new ScrapingException("Scraping not allowed by robots.txt for AtCoder");
        }
        ...
        return retry.executeSupplier(() -> {
            try {
                log.debug("Scraping AtCoder contests page: {}", config.getBaseUrl() + CONTESTS_PAGE);
                ...
                if (html == null || html.isEmpty()) {
                    throw new ScrapingException("Empty HTML content received from AtCoder");
                }
                
                Document doc = Jsoup.parse(html);
                List<ScrapedContestItem> items = new ArrayList<>();
                
                // AtCoder contests 페이지 구조에 맞는 선택자 사용
                // 실제 구조에 따라 선택자 조정 필요
                var contestElements = doc.select(".contest-card, .contest-item, .contest-row, [data-contest-id]");
                
                if (contestElements.isEmpty()) {
                    // 대안 선택자 시도
                    contestElements = doc.select("a[href*='/contests/'], table tbody tr, .table tbody tr");
                }
                ...
                log.debug("Successfully scraped {} contests from AtCoder", items.size());
                return items;
            } catch (WebClientException e) {
                log.error("Failed to fetch AtCoder contests", e);
                throw new ScrapingException("AtCoder scraping failed", e);
            } catch (Exception e) {
                log.error("Failed to scrape AtCoder contests", e);
                throw new ScrapingException("AtCoder scraping failed", e);
            }
        });
    }
    ...
    /**
     * HTML 요소에서 대회 정보 추출
     * 주의: 일본어 텍스트가 포함될 수 있음
     */
    private ScrapedContestItem extractContestInfo(Element element, String baseUrl) {
        try {
            String title = element.select("h2, h3, h4, .title, .contest-title, td").first() != null
                ? element.select("h2, h3, h4, .title, .contest-title, td").first().text()
                : element.text();
            ...
            String description = element.select(".description, .contest-description, .summary, p").first() != null
                ? element.select(".description, .contest-description, .summary, p").first().text()
                : null;
            
            // 날짜 파싱 (실제 구조에 따라 조정 필요)
            LocalDateTime startDate = null;
            LocalDateTime endDate = null;
            Element dateElement = element.select(".date, .time, .start-time, .end-time, td").first();
            if (dateElement != null) {
                try {
