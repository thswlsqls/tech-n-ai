...
| 리소스 중심 URL 설계 | ✅ 통과 | `/api/v1/contest`, `/api/v1/archive`, `/api/v1/news`, `/api/v1/sources`, `/api/v1/auth` 등 리소스 중심 설계 (단수형 사용, sources는 복수형 예외) |
...
**검증 기준**: 읽기 작업은 MongoDB Atlas에서, 쓰기 작업은 Aurora MySQL에서 수행되어야 함 (변경 이력 조회 및 삭제된 아카이브 조회는 예외적으로 Aurora MySQL에서 조회)

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| 읽기 작업 MongoDB Atlas 사용 | ✅ 통과 | 대회/뉴스/출처 목록/상세 조회, 아카이브 목록/상세 조회 모두 MongoDB Atlas |
| 쓰기 작업 Aurora MySQL 사용 | ✅ 통과 | 회원가입, 로그인, 아카이브 생성/수정/삭제 모두 Aurora MySQL |
| 변경 이력 조회 Aurora MySQL 사용 (예외) | ✅ 통과 | `GET /api/v1/history/{entityType}/{entityId}` Aurora MySQL 사용 |
| 삭제된 아카이브 조회 Aurora MySQL 사용 (예외) | ✅ 통과 | `GET /api/v1/archive/deleted` Aurora MySQL 사용 |
...
| api-archive 모듈 archive 스키마 매핑 | ✅ 통과 | `api-archive-application.yml`에서 `module.aurora.schema=archive` 설정 명시 |
...
| 애플리케이션 레벨 참조 무결성 보장 명시 | ✅ 통과 | `archives.user_id`가 `auth.users.id` 참조 시 애플리케이션 레벨에서 검증 필요 명시 |
...
| Archive 엔티티 생성 시 이벤트 발행 | ✅ 통과 | `ArchiveCreatedEvent` 명시 |
| Archive 엔티티 수정 시 이벤트 발행 | ✅ 통과 | `ArchiveUpdatedEvent` 명시 |
| Archive 엔티티 삭제 시 이벤트 발행 | ✅ 통과 | `ArchiveDeletedEvent` 명시 |
| Archive 엔티티 복원 시 이벤트 발행 | ✅ 통과 | `ArchiveRestoredEvent` 명시 |
...
**검증 기준**: 사용자 아카이브 API는 JWT 토큰에서 userId를 추출하여 해당 사용자의 아카이브만 타게팅되도록 설계되어야 함

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| JWT 토큰에서 userId 추출 명시 | ✅ 통과 | 모든 아카이브 API에 "JWT 토큰에서 userId 추출" 명시 |
| 사용자별 데이터 타게팅 | ✅ 통과 | 모든 아카이브 API가 해당 사용자의 데이터만 조회/수정/삭제 |
| 권한 검증 로직 명시 | ✅ 통과 | 본인의 아카이브만 조회/수정/삭제 가능하도록 설계 |
| 요청 body의 userId 무시 명시 | ✅ 통과 | 아카이브 추가 API에서 "요청 body의 userId는 무시하고, JWT 토큰에서 추출한 userId를 사용" 명시 |
| 중복 검증 로직 명시 | ✅ 통과 | 아카이브 추가 시 `userId + itemType + itemId` UNIQUE 제약조건으로 중복 방지 명시 |
...
**검증 기준**: 삭제된 아카이브 조회는 Aurora MySQL에서 수행되어야 함 (MongoDB는 Soft Delete 미지원)

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| 삭제된 아카이브 조회 Aurora MySQL 사용 | ✅ 통과 | `GET /api/v1/archive/deleted` Aurora MySQL 사용 명시 |
| MongoDB Soft Delete 미지원 명시 | ✅ 통과 | 설계서에 "MongoDB는 Soft Delete 미지원" 명시 |
| CQRS 패턴 예외 명시 | ✅ 통과 | "CQRS 패턴 예외"로 명시 |
| 삭제된 아카이브 조회 엔드포인트 경로 정확성 | ✅ 통과 | `/api/v1/archive/deleted` (복수형이 아닌 단수형) |
...
| 엔드포인트 경로 정확성 | ✅ 통과 | 단수형 사용 원칙 준수 (`/api/v1/archive`, `/api/v1/contest`, `/api/v1/news`, `/api/v1/auth`), `sources`는 복수형 예외 |
...
| 데이터 검증 로직 명시 | ✅ 통과 | 아카이브 추가 시 itemType, itemId 검증 명시 |
| 중복 검증 로직 명시 | ✅ 통과 | 아카이브 추가 시 중복 검증 로직 명시 |
...
| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| UNIQUE 제약조건 명시 | ✅ 통과 | 아카이브 추가 시 `userId + itemType + itemId` UNIQUE 제약조건 명시 |
...
### 1.11 사용자 아카이브 API 엔드포인트 검증

**검증 기준**: 사용자 아카이브 API 엔드포인트가 모두 정의되어야 함

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| 아카이브 추가 (`POST /api/v1/archive`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, 요청 body의 userId 무시, Aurora MySQL `Archive` 엔티티 사용, 중복 검증(userId + itemType + itemId), `ArchiveHistory` 생성(INSERT), `ArchiveCreatedEvent` 발행, api-archive 모듈 |
| 아카이브 목록 조회 (`GET /api/v1/archive`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, MongoDB Atlas `ArchiveDocument` 사용, 페이징(page, size), 정렬(sort), 필터링(itemType) 파라미터, `userId + createdAt` 또는 `userId + itemType + createdAt` 복합 인덱스 활용 |
| 아카이브 상세 조회 (`GET /api/v1/archive/{id}`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, 경로 파라미터(id: ObjectId 또는 archiveTsid), MongoDB Atlas `ArchiveDocument` 사용, 권한 검증(본인만 조회 가능) |
| 아카이브 수정 (`PUT /api/v1/archive/{id}`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, 경로 파라미터(id: archiveTsid, TSID), Aurora MySQL `Archive` 엔티티 사용, 권한 검증(본인만 수정 가능), `ArchiveHistory` 생성(UPDATE), `ArchiveUpdatedEvent` 발행 |
| 아카이브 삭제 (`DELETE /api/v1/archive/{id}`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, 경로 파라미터(id: archiveTsid, TSID), Aurora MySQL Soft Delete(is_deleted=TRUE, deleted_at, deleted_by), 권한 검증(본인만 삭제 가능), `ArchiveHistory` 생성(DELETE), `ArchiveDeletedEvent` 발행, MongoDB Atlas 물리적 삭제 |
| 아카이브 복원 (`POST /api/v1/archive/{id}/restore`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, 경로 파라미터(id: archiveTsid, TSID), Aurora MySQL 복원(is_deleted=FALSE, deleted_at=null, deleted_by=null), 권한 검증(본인만 복원 가능), `ArchiveHistory` 생성(UPDATE), `ArchiveRestoredEvent` 발행, MongoDB Atlas 새로 생성 |
| 삭제된 아카이브 목록 조회 (`GET /api/v1/archive/deleted`) | ✅ 통과 | JWT 인증 필요, JWT에서 userId 추출, Aurora MySQL 사용(CQRS 패턴 예외), 페이징(page, size), 정렬(sort: deletedAt) 파라미터, `user_id + is_deleted` 복합 인덱스 활용, MongoDB Soft Delete 미지원으로 인한 예외 |
| 변경 이력 조회 (`GET /api/v1/archive/history/{entityId}`) | ✅ 통과 | JWT 인증 필요(관리자 또는 본인), 경로 파라미터(entityId: TSID), Aurora MySQL `ArchiveHistory` 사용(CQRS 패턴 예외), 페이징, 필터링(operationType, startDate, endDate) 파라미터, `operation_type + changed_at` 복합 인덱스 활용 |
| 특정 시점 데이터 조회 (`GET /api/v1/archive/history/{entityId}/at`) | ✅ 통과 | JWT 인증 필요(관리자 또는 본인), 경로 파라미터(entityId: TSID), 쿼리 파라미터(timestamp: ISO 8601, required), Aurora MySQL 히스토리 테이블의 `before_data`/`after_data` JSON 필드 사용(CQRS 패턴 예외) |
| 특정 버전으로 복구 (`POST /api/v1/archive/history/{entityId}/restore`) | ✅ 통과 | JWT 인증 필요(관리자만), 경로 파라미터(entityId: TSID), 쿼리 파라미터(historyId: TSID, required), Aurora MySQL 사용, `after_data` JSON 필드 기반 복구, `ArchiveHistory` 생성(UPDATE), `ArchiveUpdatedEvent` 발행 |
...
| Archive 엔티티 정규화 | ✅ 통과 | User와 Many-to-One 관계, 정규화 구조 |
...
### 2.6 Archive 엔티티 동기화 전략 검증

**검증 기준**: Archive 엔티티 변경 시 즉시 ArchiveDocument로 동기화되는 전략이 수립되었는지 확인 (archiveTsid 필드 활용)

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| archiveTsid 필드 명시 | ✅ 통과 | ArchiveDocument에 archiveTsid 필드 명시 |
| 동기화 매핑 명시 | ✅ 통과 | Archive.id(TSID) → ArchiveDocument.archiveTsid 매핑 명시 |
| Kafka 이벤트 명시 | ✅ 통과 | ArchiveCreatedEvent, ArchiveUpdatedEvent, ArchiveDeletedEvent, ArchiveRestoredEvent 명시 |
| 동기화 지연 시간 명시 | ✅ 통과 | 실시간 동기화 목표 (1초 이내) 명시 |
...
### 2.7 Archive 엔티티 Soft Delete 동기화 검증

**검증 기준**: Archive 엔티티 Soft Delete 시 ArchiveDocument 물리적 삭제, 복원 시 재생성 전략이 수립되었는지 확인 (MongoDB는 Soft Delete 미지원)

| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| Soft Delete 시 물리적 삭제 명시 | ✅ 통과 | ArchiveDeletedEvent → ArchiveDocument 물리적 삭제 명시 |
| 복원 시 재생성 명시 | ✅ 통과 | ArchiveRestoredEvent → ArchiveDocument 새로 생성 명시 |
| MongoDB Soft Delete 미지원 명시 | ✅ 통과 | "MongoDB는 Soft Delete 미지원" 명시 |
...
| 검증 항목 | 상태 | 비고 |
|-----------|------|------|
| 이벤트 목록 명시 | ✅ 통과 | UserCreatedEvent, UserUpdatedEvent, UserDeletedEvent, UserRestoredEvent, ArchiveCreatedEvent, ArchiveUpdatedEvent, ArchiveDeletedEvent, ArchiveRestoredEvent 모두 명시 |
...
| Archive → ArchiveDocument 매핑 명시 | ✅ 통과 | `Archive.id(TSID)` → `ArchiveDocument.archiveTsid` 매핑 명시 |
| TSID 필드 UNIQUE 인덱스 명시 | ✅ 통과 | `userTsid`, `archiveTsid` 필드에 UNIQUE 인덱스 명시 |
...
| Archive Soft Delete 시 물리적 삭제 명시 | ✅ 통과 | `ArchiveDeletedEvent` → `ArchiveDocument` 물리적 삭제 명시 |
| Archive 복원 시 재생성 명시 | ✅ 통과 | `ArchiveRestoredEvent` → `ArchiveDocument` 새로 생성 명시 |
...
| TSID 필드 일관성 | ✅ 통과 | Phase 1 설계서의 TSID 필드(userTsid, archiveTsid)와 일치 |
...
4. ✅ 사용자 아카이브 API가 JWT 토큰 기반으로 설계됨
5. ✅ 삭제된 아카이브 조회가 Aurora MySQL에서 수행되도록 설계됨
6. ✅ Command Side와 Query Side가 명확히 분리됨
7. ✅ User/Archive 엔티티 동기화 전략이 완벽히 수립됨
...
5. ✅ **실시간 동기화**: User/Archive 엔티티 동기화 전략 완벽히 수립
