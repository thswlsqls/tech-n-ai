...
- **api-contest**: 대회 정보 조회 API (MongoDB Atlas 사용)
  ...
  - 내부 API: Batch 모듈을 위한 대회 생성/다건 생성 엔드포인트 제공
- **api-news**: 뉴스 정보 조회 API (MongoDB Atlas 사용)
  - News 데이터의 특성과 읽기 위주 요청이 필요한 비즈니스 요구사항에 따라 MongoDB Atlas 사용
  - 내부 API: Batch 모듈을 위한 뉴스 생성/다건 생성 엔드포인트 제공
- **api-chatbot**: RAG 기반 챗봇 API (MongoDB Atlas + Aurora MySQL 사용)
  - RAG 검색: MongoDB Atlas Vector Search 사용 (ContestDocument, NewsArticleDocument, ArchiveDocument)
...
**상태**: ⚠️ **미구현** - 현재 구현되지 않았습니다. `SourcesDocument`는 MongoDB에 정의되어 있으나, API 엔드포인트는 아직 구현되지 않았습니다.
...
#### 4. 대회 생성 (내부 API, Batch 모듈 전용)

**엔드포인트**: `POST /api/v1/contest/internal`

**설명**: Batch 모듈에서 대회 데이터를 MongoDB Atlas에 저장하기 위한 내부 API입니다.

**인증**: 내부 API 키 (`X-Internal-Api-Key` 헤더)

**요청 헤더**:
- `X-Internal-Api-Key` (String, required): 내부 API 키
...
1. 내부 API 키 검증
2. 데이터 검증 및 중복 체크
3. `ContestDocument` 생성 및 저장 (MongoDB Atlas)
4. 트랜잭션 관리: `@Transactional` 사용 (단건 처리)

**응답 예시**: 대회 상세 조회와 동일

#### 5. 대회 다건 생성 (내부 API, Batch 모듈 전용)

**엔드포인트**: `POST /api/v1/contest/internal/batch`

**설명**: Batch 모듈에서 대회 데이터를 다건으로 MongoDB Atlas에 저장하기 위한 내부 API입니다.
...
**처리 로직**:
1. 내부 API 키 검증
2. 각 항목에 대해 단건 처리 Service 메서드 반복 호출
3. 부분 롤백 구현: 실패한 항목은 롤백, 성공한 항목은 정상 커밋
4. Facade 레이어에는 `@Transactional` 사용하지 않음 (각 단건 처리가 독립적인 트랜잭션)
...
#### 4. 뉴스 생성 (내부 API, Batch 모듈 전용)

**엔드포인트**: `POST /api/v1/news/internal`

**설명**: Batch 모듈에서 뉴스 데이터를 MongoDB Atlas에 저장하기 위한 내부 API입니다.
...
1. 내부 API 키 검증
2. 데이터 검증 및 중복 체크
3. `NewsArticleDocument` 생성 및 저장 (MongoDB Atlas)
4. 트랜잭션 관리: `@Transactional` 사용 (단건 처리)

**응답 예시**: 뉴스 상세 조회와 동일

#### 5. 뉴스 다건 생성 (내부 API, Batch 모듈 전용)

**엔드포인트**: `POST /api/v1/news/internal/batch`

**설명**: Batch 모듈에서 뉴스 데이터를 다건으로 MongoDB Atlas에 저장하기 위한 내부 API입니다.
...
**설명**: Full-text search를 사용하여 아카이브를 검색합니다.
...
**데이터 소스**: MongoDB Atlas (`ArchiveDocument`, Full-text search)

**처리 로직**:
1. JWT 토큰에서 `userId` 추출
2. `ArchiveDocument` 검색 (MongoDB Atlas, userId 필터링, Full-text search)
3. 페이징 및 정렬 적용

**응답 형식**: 아카이브 목록 조회와 동일
...
**데이터 소스**: 
- MongoDB Atlas (RAG 검색: ContestDocument, NewsArticleDocument, ArchiveDocument)
...
- 뉴스 목록/상세/검색
- 출처 목록 조회 (미구현)
- 아카이브 목록/상세/검색 조회 (활성 아카이브만)
- 챗봇 RAG 검색 (ContestDocument, NewsArticleDocument, ArchiveDocument)
...
- 대회 생성/다건 생성 (Batch 모듈 전용)
- 뉴스 생성/다건 생성 (Batch 모듈 전용)
