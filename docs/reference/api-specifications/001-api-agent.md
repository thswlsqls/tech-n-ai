...
분석 Tool(`get_emerging_tech_statistics`, `analyze_text_frequency`) 실행 시 구조화된 차트 데이터가 자동으로 수집되어 `chartData` 배열에 포함됩니다.
...
- `summary`의 Mermaid 블록과 `chartData`는 동일한 데이터를 다른 형태로 표현한 것이므로, 프론트엔드에서 `chartData`가 존재할 때 Mermaid 블록을 숨기거나 자체 차트로 대체할 수 있음
- 한 요청에서 여러 분석 Tool이 호출되면 `chartData`에 복수의 항목이 포함됨
