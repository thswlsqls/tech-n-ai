...
| `api-archive` | ✅ 사용 | `archive` | archives, archive_history |
...
2. **domain/aurora/src/main/resources/application-api-domain.yml**에서 자동으로 사용:
```yaml
spring:
  datasource:
    writer:
...
**관리 테이블**:
- `archives`: 사용자 아카이브 정보
- `archive_history`: 아카이브 변경 이력

**설정 방법**:

1. **api-archive 모듈의 `api-archive-application.yml` 파일**:
```yaml
module:
  aurora:
    schema: archive
...
**설명**: `api-archive` 모듈은 `module.aurora.schema=archive`로 설정하면, `application-api-domain.yml`의 `${module.aurora.schema}`가 `archive`로 치환되어 `archive` 스키마에 연결됩니다.

**주의사항**:
- `archives` 테이블의 `user_id`는 `auth` 스키마의 `users` 테이블을 참조합니다.
...
**domain/aurora/src/main/resources/application-api-domain.yml** 파일은 모든 API 모듈에서 공통으로 사용되며, `${module.aurora.schema}` 환경변수를 통해 동적으로 스키마를 참조합니다:
...
```sql
-- user_id 외래 키 인덱스
CREATE INDEX idx_archive_user_id ON archives (user_id);

-- user_id + is_deleted 복합 인덱스
CREATE INDEX idx_archive_user_is_deleted ON archives (user_id, is_deleted);

-- user_id + item_type + item_id UNIQUE 복합 인덱스 (중복 방지)
CREATE UNIQUE INDEX idx_archive_user_item ON archives (user_id, item_type, item_id);
```

#### Foreign Key

**주의사항**: `archives` 테이블은 `archive` 스키마에 있으며, `users` 테이블은 `auth` 스키마에 있습니다. MySQL은 스키마 간 Foreign Key 제약조건을 지원하지 않으므로, 다음 Foreign Key 제약조건은 실제로는 생성되지 않습니다. 대신 애플리케이션 레벨에서 참조 무결성을 보장해야 합니다.
...
Archive 테이블의 변경 이력을 저장하는 테이블입니다.
...
```sql
-- archive_id 외래 키 인덱스
CREATE INDEX idx_archive_history_archive_id ON archive_history (archive_id);

-- changed_at 인덱스
CREATE INDEX idx_archive_history_changed_at ON archive_history (changed_at);

-- operation_type + changed_at 복합 인덱스
CREATE INDEX idx_archive_history_operation ON archive_history (operation_type, changed_at);
```
...
ADD CONSTRAINT fk_archive_history_archive
...
| `archives` | `user_id` | 외래 키 | User 참조 |
| `archives` | `user_id, is_deleted` | 복합 | 사용자별 활성 아카이브 조회 |
| `archives` | `user_id, item_type, item_id` | UNIQUE | 중복 아카이브 방지 |
...
| `archive_history` | `archive_id` | 외래 키 | Archive 참조 |
| `archive_history` | `changed_at` | 단일 | 변경 일시 정렬 |
| `archive_history` | `operation_type, changed_at` | 복합 | 작업 타입별 변경 이력 조회 |
...
erDiagram
    ...
    ARCHIVES ||--o{ ARCHIVE_HISTORY : "has"
...
2. **USERS → ARCHIVES**: One-to-Many (한 사용자는 여러 아카이브를 가질 수 있음)
...
7. **ARCHIVES → ARCHIVE_HISTORY**: One-to-Many (한 아카이브는 여러 변경 이력을 가질 수 있음)
...
- **v2.0 (2026-01-07)**: 
  ...
  - DDL 예제에 스키마 지정 추가
  - api-auth, api-archive 모듈의 스키마 분리 명시
  - api-contest, api-news 모듈의 MongoDB Atlas 사용 명시
