...
- **스키마 분리**: `auth`, `bookmark`, `chatbot` 스키마로 모듈별 데이터 격리
...
1. **Initial database name**
   ...
   - 나중에 `bookmark`, `chatbot` 스키마 추가 생성 가능
...
5. **추가 스키마 생성**
   - `bookmark`, `chatbot` 스키마도 동일한 방법으로 생성
...
```yaml
# api/bookmark/src/main/resources/application.yml
module:
  aurora:
    schema: bookmark
```
