...
```javascript
{
  ...
  pros: [String],                   // 장점 목록
  cons: [String],                   // 단점 목록
  implementationDifficulty: String, // 구현 난이도: "Easy", "Medium", "Hard"
  cost: String,                     // 비용: "Free", "Paid", "Freemium"
  costDetails: String,              // 비용 상세 정보
  recommendedUseCase: String,       // 권장 사용 사례
  integrationExample: String,       // 통합 예제
  alternativeSources: [String],     // 대체 출처 목록
...
// URL로 출처 조회
db.sources.findOne({ url: "https://codeforces.com" })
```
...
db.archives.createIndex({ userId: 1, itemStartDate: -1 })

// 7. userId + itemEndDate 복합 인덱스 (ESR 규칙)
// Equality: userId, Sort: itemEndDate (Contest 정렬용)
db.archives.createIndex({ userId: 1, itemEndDate: -1 })

// 8. userId + itemPublishedAt 복합 인덱스 (ESR 규칙)
// Equality: userId, Sort: itemPublishedAt (News 정렬용)
db.archives.createIndex({ userId: 1, itemPublishedAt: -1 })

// 9. userId + archivedAt 복합 인덱스 (ESR 규칙)
// Equality: userId, Sort: archivedAt
db.archives.createIndex({ userId: 1, archivedAt: -1 })
```
...
```javascript
{
  ...
  status: String,                   // 상태: "PUBLISHED", "DRAFT", "ARCHIVED"
  metadata: {                       // 메타데이터
...
| `archives` | `itemId` | 단일 | - |
| `archives` | `userId, itemStartDate` | 복합 | E, S |
| `archives` | `userId, itemEndDate` | 복합 | E, S |
| `archives` | `userId, itemPublishedAt` | 복합 | E, S |
| `archives` | `userId, archivedAt` | 복합 | E, S |
...
3. **CONTESTS → ARCHIVES**: One-to-Many (한 대회는 여러 사용자에 의해 아카이브될 수 있음)
4. **NEWS_ARTICLES → ARCHIVES**: One-to-Many (한 뉴스 기사는 여러 사용자에 의해 아카이브될 수 있음)
...
```javascript
{
  ...
  cost: "Free",
  costDetails: "완전 무료",
  recommendedUseCase: "알고리즘 대회 일정 및 결과 조회, 대회 정보 실시간 모니터링",
  integrationExample: "GET https://codeforces.com/api/contest.list?gym=false",
  alternativeSources: ["AtCoder API", "LeetCode Contests"],
  enabled: true,
...
```json
{
  ...
  "rate_limit": "No strict rate limit, but recommends reasonable usage",
  ...
  "cost": "Free",
  "cost_details": "완전 무료",
  "recommended_use_case": "알고리즘 대회 일정 및 결과 조회, 대회 정보 실시간 모니터링",
  "integration_example": "GET https://codeforces.com/api/contest.list?gym=false",
  "alternative_sources": ["AtCoder API", "LeetCode Contests"],
  "enabled": true,
...
```json
{
  ...
  "embedding_text": "Codeforces Round 900 Regular Codeforces contest algorithm competitive-programming",
...
```json
{
  ...
  "embedding_text": "Spring Boot 4.0 Released Spring Boot 4.0 brings significant improvements... Spring Boot 4.0 has been released with new features...",
  "embedding_vector": null,
...
```json
{
  ...
  "archive_tsid": "1234567890123456789",
  "user_id": "user123",
  "item_type": "CONTEST",
  ...
  "tag": "algorithm",
  "memo": "참가 예정",
  "archived_at": "2026-01-07T10:00:00Z",
...
### SourcesDocument
- 예상 크기: 약 1 KB ~ 2 KB (pros, cons, alternative_sources 배열 포함)
