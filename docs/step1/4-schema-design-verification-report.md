...
| archive_history | BIGINT UNSIGNED | ✅ | ✅ 통과 |
...
- ✅ `archives` 테이블에 `user_id + is_deleted` 복합 인덱스 생성
...
**검증 항목**:
- ⚠️ `archives` 테이블의 `user_id + item_type + item_id` UNIQUE 제약조건은 Soft Delete를 고려하여 부분 인덱스로 구현 필요
...
| 컬렉션 | 인덱스 | ESR 규칙 | 상태 |
|--------|--------|----------|------|
| sources | category + priority | E, S | ✅ 통과 |
| contests | sourceId + startDate | E, S | ✅ 통과 |
| contests | status + startDate | E, S | ✅ 통과 |
| news_articles | sourceId + publishedAt | E, S | ✅ 통과 |
| archives | userId + createdAt | E, S | ✅ 통과 |
| archives | userId + itemType + createdAt | E, E, S | ✅ 통과 |
| exception_logs | source + occurredAt | E, S | ✅ 통과 |
| exception_logs | exceptionType + occurredAt | E, S | ✅ 통과 |

**검증 결과**:
- ✅ 모든 복합 인덱스가 ESR 규칙을 준수
- ✅ Equality → Sort → Range 순서 준수
- ✅ TTL 인덱스 적절히 사용 (news_articles, exception_logs)
...
| users → archives | users | archives | ON DELETE CASCADE | ✅ 통과 |
...
| archives → archive_history | archives | archive_history | ON DELETE CASCADE | ✅ 통과 |
...
**검증 결과**:
- ✅ `ContestDocument`와 `NewsArticleDocument`의 `sourceId`가 `SourcesDocument` 참조
- ✅ `ArchiveDocument`의 `itemId`가 MongoDB Atlas ObjectId 참조
- ✅ 참조 관계가 ObjectId로 명시됨
...
**개선 사항**:
1. ⚠️ `archives` 테이블의 UNIQUE 제약조건을 부분 인덱스로 구현 필요 (Soft Delete 고려)
