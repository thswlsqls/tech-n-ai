...
`domain/mongodb/src/main/java/com/tech/n/ai/domain/mongodb/config/MongoIndexConfig.java` 파일이 애플리케이션 시작 시 다음 인덱스를 자동 생성합니다:
...
- **bookmarks**: `user_id + created_at`, `user_id + item_type + created_at`, `user_id + item_type + item_id` (UNIQUE), 등
...
// bookmarks 컬렉션 인덱스 생성
db.bookmarks.createIndex({ bookmark_tsid: 1 }, { unique: true })
db.bookmarks.createIndex({ user_id: 1, created_at: -1 })
db.bookmarks.createIndex({ user_id: 1, item_type: 1, created_at: -1 })
db.bookmarks.createIndex({ user_id: 1, item_type: 1, item_id: 1 }, { unique: true })
db.bookmarks.createIndex({ item_id: 1 })
...
- **BookmarkDocument**: 사용자 북마크 항목 검색
...
**ContestDocument** (`domain/mongodb/src/main/java/com/tech/n/ai/domain/mongodb/document/ContestDocument.java`):
```java
...
**NewsArticleDocument** (`domain/mongodb/src/main/java/com/tech/n/ai/domain/mongodb/document/NewsArticleDocument.java`):
```java
@Field("embedding_text")
private String embeddingText;  // 임베딩 대상 텍스트
...
**BookmarkDocument** (`domain/mongodb/src/main/java/com/tech/n/ai/domain/mongodb/document/BookmarkDocument.java`):
```java
@Field("embedding_text")
private String embeddingText;  // 임베딩 대상 텍스트
...
**BookmarkDocument Vector Index 생성**:
```bash
atlas clusters search indexes create \
  --clusterName tech-n-ai-cluster \
  --collectionName bookmarks \
  --databaseName tech_n_ai \
  --name vector_index_bookmarks \
  --definition '{
    "fields": [
      {
        "type": "vector",
...
**참고**: 실제 구현은 `api/chatbot/src/main/java/com/tech/n/ai/api/chatbot/service/VectorSearchServiceImpl.java`를 참고하세요.
...
- **BookmarkDocument**: `user_id` 필터 (사용자별 검색)
...
`domain/mongodb/src/main/java/com/tech/n/ai/domain/mongodb/config/MongoClientConfig.java` 파일에서 다음 설정이 적용됩니다:
...
```javascript
// 쿼리 실행 계획 확인
db.bookmarks.find({ user_id: "user123" }).sort({ created_at: -1 }).explain("executionStats")
