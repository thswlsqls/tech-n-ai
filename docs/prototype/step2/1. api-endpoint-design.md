...
- **api-chatbot**: RAG 기반 챗봇 API (MongoDB Atlas + Aurora MySQL 사용)
  - RAG 검색: MongoDB Atlas Vector Search 사용 (EmergingTechDocument, BookmarkDocument)
...
**스키마 설정**:
- `api-bookmark-application.yml`에서 `module.aurora.schema=bookmark` 설정
- `domain/aurora/src/main/resources/application-api-domain.yml`에서 `${module.aurora.schema}` 사용하여 동적으로 `bookmark` 스키마에 연결
...
**데이터 소스**: 
- MongoDB Atlas (RAG 검색: ContestDocument, NewsArticleDocument, BookmarkDocument)
...
- 챗봇 RAG 검색 (ContestDocument, NewsArticleDocument, BookmarkDocument)
