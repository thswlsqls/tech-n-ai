...
- LangChain4j Agent의 자율 실행 (9개 Tool)
- Tool 선택 및 중복 검증
...
3. Tool 선택: collect_github_releases("openai", "openai-python")
   → 결과: GitHub 릴리스 수집 후 DB 저장 (신규 3건, 중복 12건)

4. Tool 선택: collect_scraped_articles("ANTHROPIC")
   → 결과: Anthropic 블로그 크롤링 후 DB 저장 (신규 2건)
...
| `send_slack_notification` | Slack 채널에 관리자 알림 메시지 전송 | 알림 |

#### 3. 멀티 턴 대화 및 세션 관리
- **세션 기반 대화**: 동일 `sessionId`로 요청하면 이전 대화 맥락을 유지하는 멀티 턴 대화 지원
- **메모리 관리**: `MongoDbChatMemoryStore` 기반 영속 저장, 세션당 최대 30개 메시지 인메모리 윈도우
- **대화 영속화**: Aurora MySQL(Command) + MongoDB(Query)에 CQRS 패턴으로 모든 대화 저장 (`common-conversation` 모듈)
- **세션 타이틀 자동생성**: 첫 대화 완료 후 `@Async` 비동기 LLM 호출로 3~5단어 타이틀 자동 생성
- **세션 관리 API**: 세션 목록/상세 조회, 대화 이력 조회, 세션 삭제, 타이틀 수동 변경

#### 4. ChartData 구조화 응답
- **분석 Tool 실행 시 자동 수집**: `get_emerging_tech_statistics`, `analyze_text_frequency` 호출 시 구조화된 차트 데이터 반환
- **프론트엔드 직접 활용**: `chartType`(`pie`/`bar`), `dataPoints`(`label`/`value`) 구조로 차트 라이브러리에 직접 전달 가능
- **Mermaid와 병행**: `summary`의 Mermaid 블록과 동일 데이터를 구조화된 형태로 제공

#### 5. 안전한 실행 보장
- **3종 Error Handler**: Tool 실행 오류, Tool 인자 오류, Hallucinated Tool Name 각각에 대한 전용 핸들러
- **입력값 검증**: `ToolInputValidator`를 통한 LLM hallucination 방어 (잘못된 Provider, 날짜 형식 등)
...
AI Agent는 REST API(ADMIN JWT 인증) 또는 Scheduler를 통해 트리거되며, AgentFacade를 거쳐 LangChain4j AiServices를 활용하여 OpenAI GPT-4o-mini와 통신합니다. Agent는 9개의 Tool을 사용하여 GitHub API, 웹 페이지, api-emerging-tech API, MongoDB Atlas(Aggregation 기반 통계/빈도 분석), Slack과 상호작용합니다. 조회/분석 뿐 아니라 자율적으로 데이터를 수집하여 MongoDB에 저장하는 기능도 제공합니다.

AgentFacade는 사용자 메시지/Agent 응답을 `common-conversation` 모듈을 통해 Aurora MySQL + MongoDB에 CQRS 패턴으로 영속화하고, 새 세션 생성 시 비동기로 세션 타이틀을 자동 생성합니다. 분석 Tool 실행 시 `ToolExecutionMetrics`가 구조화된 `ChartData`를 수집하여 응답에 포함하므로, 프론트엔드에서 차트 컴포넌트로 직접 시각화할 수 있습니다.
...
> `chartData`가 빈 배열이 아닌 경우, 프론트엔드에서 `chartType`에 따라 차트 컴포넌트로 시각화할 수 있습니다. `summary`의 Mermaid 블록과 동일 데이터를 구조화된 형태로 제공합니다.
...
| `AI_UPDATE_INTERNAL_API_KEY` | emerging-tech 내부 API 인증 키 | Yes |
...
```
api/
├── agent/                    # AI Agent 모듈 (Port 8086)
│   ├── agent/
│   │   ├── EmergingTechAgent.java        # Agent 인터페이스
│   │   ├── EmergingTechAgentImpl.java    # Agent 구현체 (루프 감지, 에러 처리)
│   │   ├── AgentAssistant.java           # LangChain4j AiServices 인터페이스
│   │   ├── AgentExecutionResult.java     # 실행 결과 DTO (ChartData 포함)
│   │   └── dto/
│   │       └── ChartData.java            # 구조화된 차트 데이터 DTO
│   ├── config/
│   │   ├── AiAgentConfig.java
...
│   │   └── ServerConfig.java
│   ├── controller/
│   │   └── AgentController.java          # REST API (Agent 실행 + 세션 관리)
│   ├── dto/
│   │   ├── request/
│   │   │   ├── AgentRunRequest.java
│   │   │   ├── AgentSessionListRequest.java
│   │   │   ├── AgentMessageListRequest.java
│   │   │   └── UpdateSessionTitleRequest.java
│   │   └── response/
│   │       ├── AgentSessionListResponse.java
│   │       └── AgentMessageListResponse.java
│   ├── exception/
│   │   └── AgentLoopDetectedException.java  # 루프 감지 예외
│   ├── facade/
│   │   └── AgentFacade.java              # Controller-Agent 오케스트레이션
│   ├── metrics/
│   │   └── ToolExecutionMetrics.java     # ThreadLocal 기반 실행 메트릭
│   ├── scheduler/
│   │   └── EmergingTechAgentScheduler.java
│   ├── service/
│   │   ├── GitHubDataCollectionService.java
│   │   ├── RssDataCollectionService.java
│   │   ├── ScraperDataCollectionService.java
│   │   ├── SessionTitleGenerationService.java      # 세션 타이틀 자동생성 인터페이스
│   │   └── SessionTitleGenerationServiceImpl.java  # @Async 비동기 구현
│   └── tool/
│       ├── EmergingTechAgentTools.java   # 9개 Tool 메서드 (입력 검증 + 루프 방어)
│       ├── adapter/
...
│       ├── dto/                          # Tool 입출력 DTO
│       ├── handler/
│       │   └── ToolErrorHandlers.java    # 3종 에러 핸들러
│       ├── util/
│       │   ├── DataCollectionProcessorUtil.java
│       │   └── TextTruncator.java
│       └── validation/
│           └── ToolInputValidator.java   # LLM hallucination 방어 검증
│
└── emerging-tech/            # Emerging Tech API 모듈 (Port 8082)
    ├── controller/
...
- **Rate Limiting**: Redis Token Bucket 기반 경로별 요청 제한 (IP 기반 / 사용자 기반)
- **요청 추적 (Request ID)**: `X-Request-Id` 헤더를 통한 분산 요청 추적, UUID 검증 및 자동 생성
- **Access Log**: 구조화된 접근 로그 기록 (HTTP 메서드, 경로, 상태코드, 응답시간, 클라이언트 IP, 사용자 ID, 라우트 ID)
- **Retry & Request Size 제한**: GET 요청 자동 재시도 (2회, 지수 백오프), 요청 크기 제한 (5MB)
- **Global CORS 설정**: 모든 경로에 대한 CORS 정책 적용, 환경별 차별화, 중복 헤더 자동 제거
...
API Gateway (Spring Cloud Gateway)
  ...
  └─ /api/v1/agent/** → @api/agent (ADMIN 인증 필요)
```
...
│   └── RateLimiterConfig.java                 # Redis Token Bucket Rate Limiter 설정
├── filter/
...
└── src/main/resources/
    ├── application.yml                        # 기본 설정 (라우팅, Rate Limiting, 연결 풀, CORS)
...
- [관리자 인증 보안 강화 설계서](docs/reference/admin-auth-security-improvement-prompt.md)
...
- **인증 필요**: `/api/v1/bookmark/**`, `/api/v1/chatbot/**`, `/api/v1/agent/**` (ADMIN)
...
#### 기타 설계
- [MySQL Docker 로컬 환경 구축 가이드](docs/reference/mysql-docker-local-setup-guide.md)
