package com.tech.n.ai.batch.source.domain.contest.mlh.processor;
 ...
 * 
 * Major League Hacking (MLH) 공식 문서 참고:
 * https://mlh.io/events
 * 
...
public class MLHStep1Processor implements ItemProcessor<ScrapedContestItem, ContestCreateRequest> {
    ...
    public @Nullable ContestCreateRequest process(ScrapedContestItem item) throws Exception {
        if (item == null) {
            log.warn("MLH scraped contest item is null");
            return null;
        }

        // 필수 필드 검증
        if (item.title() == null || item.title().isBlank()) {
            log.warn("MLH scraped contest item title is null or blank, skipping item: {}", item.url());
            return null;
        }

        // 날짜/시간 검증
        if (item.startDate() == null) {
            log.warn("MLH scraped contest item startDate is null, skipping item: {}", item.title());
            return null;
        }

        if (item.endDate() == null) {
            log.warn("MLH scraped contest item endDate is null, skipping item: {}", item.title());
            return null;
        }
        ...
        if (url == null || url.isBlank()) {
            log.warn("MLH scraped contest item url is null or blank, skipping item: {}", item.title());
            return null;
        }
        ...
        ContestCreateRequest.ContestMetadataRequest metadata = ContestCreateRequest.ContestMetadataRequest.builder()
            .sourceName("MLH")
            .prize(item.prize())
            .participants(null) // MLH에서 제공하지 않음
            .tags(tags)
            .build();
