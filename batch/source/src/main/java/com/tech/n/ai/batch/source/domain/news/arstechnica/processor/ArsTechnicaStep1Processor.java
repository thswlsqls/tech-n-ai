package com.tech.n.ai.batch.source.domain.news.arstechnica.processor;
...
/**
 * Ars Technica Step1 Processor
 * RssFeedItem → NewsCreateRequest 변환
 * 
 * Ars Technica RSS 피드 공식 문서 참고:
...
public class ArsTechnicaStep1Processor implements ItemProcessor<RssFeedItem, NewsCreateRequest> {

    /**
     * Ars Technica 출처의 sourceId
     * TODO: SourcesDocument에서 Ars Technica 출처의 ID를 조회하도록 구현 필요
     */
    private static final String ARS_TECHNICA_SOURCE_ID = "507f1f77bcf86cd799439023";

    @Override
    public @Nullable NewsCreateRequest process(RssFeedItem item) throws Exception {
        if (item == null) {
            log.warn("Ars Technica RSS feed item is null");
            return null;
        }

        // 필수 필드 검증
        if (item.title() == null || item.title().isBlank()) {
            log.warn("Ars Technica RSS feed item title is null or blank, skipping item: {}", item.guid());
            return null;
        }

        // 날짜/시간 검증
        if (item.publishedDate() == null) {
            log.warn("Ars Technica RSS feed item publishedDate is null, skipping item: {}", item.title());
            return null;
        }
        ...
        if (url == null || url.isBlank()) {
            log.warn("Ars Technica RSS feed item url is null or blank, skipping item: {}", item.title());
            return null;
        }
        ...
        return NewsCreateRequest.builder()
            .sourceId(ARS_TECHNICA_SOURCE_ID)
            .title(item.title())
            .content(content)
            .summary(summary)
