...
3. **데이터 수집 전략** (유지 - RSS 관련 부분만 검토)
4. **구현 가이드** (개선: Spring Boot 베스트 프랙티스 반영 - client-rss 부분만)
5. **참고 자료** (확장: 모든 참고 출처 정리)

**참고**: 이 프롬프트는 `client-rss` 모듈만 다룹니다. `client-scraper` 모듈은 별도 프롬프트로 처리됩니다.
