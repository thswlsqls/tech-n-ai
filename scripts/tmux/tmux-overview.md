...
tmux(terminal multiplexer)는 하나의 터미널에서 여러 터미널 세션을 생성, 관리, 전환할 수 있는 도구이다. ISC 라이선스로 배포되며, OpenBSD 프로젝트에서 시작되었다.
...
- 수평 분할(`Ctrl-b %`, 좌우), 수직 분할(`Ctrl-b "`, 상하)이 가능하다.
...
tmux split-window -h -t session:window -c /path   # 수평 분할 (좌우)
tmux split-window -v -t session:window -c /path   # 수직 분할 (상하)
